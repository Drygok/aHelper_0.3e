...
namespace aHelper
{
    class Memory
    {
        ...
        #region Константы
        public IntPtr POINTER_CPED = (IntPtr)0xB6F5F0;
        public IntPtr POINTER_VEHICLE = (IntPtr)0xBA18FC;
        public IntPtr ADDR_CPED_MONEY = (IntPtr)0xB7CE50;
        public int OFFSET_SETPOSITION = 0x14;
        public int OFFSET_CPED_HEALTH = 0x540;
        public int OFFSET_VEHICLE_HEALTH = 0x4C0;
        public string[] ERR = { "OK", "ПРОЦЕСС НЕ НАЙДЕН", "ОШИБКА ИНИЦИАЛИЗАЦИИ", "ОШИБКА CH" };
        #endregion
            ...
            foreach (Process process in proc)
                if (process.ProcessName == "grand_theft_auto_san_andreas.dll")
                {
                    ...
                    foreach (ProcessModule i in modules)
                    {
                        ...
                        if (GameID && GTA && SAMP) return true;
                    }
                    return false;
                }
            return false;
